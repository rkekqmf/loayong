	...
	{#if profileData}
		...
		{:else}
			<div class="absolute right-3 top-3 flex gap-2">
				<!-- 캐릭터 수정 버튼 -->
				<button aria-label="캐릭터 수정" on:click={() => (isCharacterModalOpen = true)} class="block rounded-full bg-blue-500 p-1 text-white transition-colors hover:bg-blue-300">
				...
				<!-- 기존 리셋 버튼 -->
				<button aria-label="초기화" on:click={handleReset} class="block rounded-full bg-red-500 p-1 text-white transition-colors hover:bg-red-300">
			...
			<div class="slider-container">
				<div class="slider-wrapper">
					{#each sortedCharacters as character, i (character.CharacterName)}
						...
						>
							<div class="bg-app-box-bg flex flex-col gap-3 rounded-md p-2 text-white">
...
{#if isModalOpen && selectedCharacter}
	<div transition:slide={{ duration: 300, easing: quartOut }} class="absolute left-0 right-0 top-[90px] z-20 border-t-2 border-app-box-border bg-gradient-to-tr from-[#2c402f]/55 to-[#354f34]/95 p-3 shadow-lg backdrop-blur-sm">
			...
			{#each profileData as character (character.CharacterName)}
				...
				>
					...
					<button class="relative flex flex-1 items-center gap-2" on:click={() => toggleCharacterSelection(character)}>
						<img src={`/class/${getClassCode(character.CharacterClassName)}.png`} alt={character.CharacterClassName} class="h-6 w-6 rounded-full border border-app-box-border object-cover p-1 [filter:brightness(0)_invert(1)]" />
